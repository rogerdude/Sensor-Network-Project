...
K_THREAD_STACK_DEFINE(mqtt_stack_area, 1024);
...
const struct json_obj_descr geo_descr[] = {
...
static lv_point_precise_t vline_pts[COLS + 1][2];
static lv_point_precise_t hline_pts[ROWS + 1][2];

/* Forward: update cell from lat/lon */
static void update_from_geo(float lat, float lon, int severity);

static struct mqtt_topic subscribe_topics[] = {
    {
...
static void mqtt_evt_handler(struct mqtt_client *c, const struct mqtt_evt *evt) {
    ...
    case MQTT_EVT_PUBLISH: {
        ...
        if (ret < 0) {
            ...
            /* success! data.longitude, data.latitude, data.severity are set */
            update_from_geo(data.latitude,
                            data.longitude,
                            data.severity);
        }
        break;
